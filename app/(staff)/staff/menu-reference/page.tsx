...
export default function StaffMenuReferencePage() {
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "16px", marginBottom: "24px" }}>
        {[
          ...
          { label: "Available", value: items.filter((i) => i.is_available).length, icon: "✅", color: "#22c55e" },
          { label: "Unavailable", value: items.filter((i) => !i.is_available).length, icon: "❌", color: "#ef4444" },
        ].map((s) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "14px" }}>
              {catItems.map((item) => (
                <div key={item.id} style={{ background: "#fff", borderRadius: "12px", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", overflow: "hidden", opacity: item.is_available ? 1 : 0.6, border: item.is_available ? "none" : "1px solid #fecaca" }}>
                  <div style={{ position: "relative", height: "130px", background: "#f1ebe5" }}>
                    {item.image_url
                    ...
                    <div style={{ position: "absolute", top: "8px", right: "8px", display: "flex", gap: "4px" }}>
                      ...
                      {item.is_featured && <span style={{ fontSize: "10px", fontWeight: 800, padding: "3px 8px", borderRadius: "20px", background: "#fef3c7", color: "#d97706" }}>⭐</span>}
                    </div>
                  </div>
                  <div style={{ padding: "12px 14px" }}>
                    ...
                    <div style={{ fontWeight: 900, fontSize: "15px", color: ACCENT }}>₱{Number(item.price).toFixed(2)}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {filtered.length === 0 && <div style={{ textAlign: "center", padding: "60px", color: "#aaa", fontSize: "14px" }}>No items found.</div>}
    </div>
  );
}
